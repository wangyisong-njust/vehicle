...
def plot_obb_st_lstm_ablation(ablation: dict, main_metrics: dict, out_path: Path) -> None:
    ...
    for name, item in ablation.items():
        ...
        if not m:
            continue
        names.append(name)
        vals.append(float(m.get("f1_macro", 0.0)))
    colors = ["#d62728"] + ["#4c78a8"] * (len(names) - 1)
    fig, ax = plt.subplots(figsize=(9, 4.5))
    ax.bar(np.arange(len(names)), vals, color=colors, edgecolor="black", linewidth=0.5)
    ax.set_xticks(np.arange(len(names)))
...
def write_report(root: Path) -> None:
    ...
    obb_st_vs_lstm = obb_st_future["f1_macro"] - lstm_future["f1_macro"]
    obb_st_vs_best_baseline = obb_st_future["f1_macro"] - max(
        xgb_future["f1_macro"], xgb_temporal_future["f1_macro"], lstm_future["f1_macro"], gru_future["f1_macro"]
    )
    ...
    delta_word = "提升" if obb_st_vs_best_baseline >= 0 else "低于"
    prediction_note = (
    ...
    if obb_st_future["f1_macro"] < 0.50:
        prediction_note += "未来预测 Macro-F1 低于当前状态识别主要受 324 时间窗和后 30% 测试段类别失衡限制，论文应同步给出混淆矩阵与类别支持数。"
    ...
    obb_st_input_shape = obb_st.get("input_shape") or obb_st_meta.get("input_shape", [0, 0, 0, 0, 0])
    grid_shape = obb_st_meta.get("grid_shape", [4, 12])
    lines.extend(
        [
            "",
            "## 1.5 未来状态预测",
            ...
            (
                f"模型输入张量形状为 (T, C, H, W) = "
                f"({obb_st_input_shape[1] if len(obb_st_input_shape) > 1 else '-'}, "
                f"{obb_st_input_shape[2] if len(obb_st_input_shape) > 2 else '-'}, "
                f"{grid_shape[0]}, {grid_shape[1]})，"
                f"训练序列 {obb_st_meta.get('train_sequences', '-')} 条，"
                f"测试序列 {obb_st_meta.get('test_sequences', '-')} 条。"
                "卷积通道为 (8, 8)，LSTM 隐元 64，FocalLoss(γ=2.0) 处理类别不平衡，5 种子概率集成。"
            ),
            "",
    ...
    for abl_name, abl_data in obb_st_ablation.items():
        ...
        if not m:
            continue
        lines.append(f"| {abl_name} | 见注 | {m['f1_macro']:.4f} | {m['accuracy']:.4f} |")
    lines.extend(
        [
            "",
    ...
    if horizon_sweep.get("results"):
        ...
        accuracy_lead = 0
        f1_lead = 0
        for item in horizon_sweep["results"]:
            if item.get("status") != "completed":
                lines.append(f"| {item.get('horizon_seconds', '-'):.1f} | (skipped) | — | — | — | {item.get('reason', '')} |")
                continue
            m = item["models"]
            f1s = {k: v["f1_macro"] for k, v in m.items()}
            ...
            def fmt_acc(name, val):
                ...
                return f"{val:.4f}" + ("★" if strong else "")
            lines.append(
                f"| {item['horizon_seconds']:.1f} | Macro-F1 | "
