...
为和近年长时交通预测论文形成同口径补充，额外在 `PEMS08` 上做 15/30/60 分钟交通流预测。该数据包含 17856 个 5 分钟时间步、170 个检测器，预测对象为 traffic flow。这个实验不使用 pixel 表或车辆框，只用于证明本文报告覆盖短时状态预测和长时交通流预测两类任务。
