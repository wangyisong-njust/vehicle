...
（1）1 号场景（V = 21.23 km·h⁻¹，D = 0.571 veh·m⁻¹，N = 80）：该时段平均车速在五个场景中最低、车辆密度最高，箭头所示同一车辆在 5 s 内位移很小，已接近拥挤与堵塞的判别边界。状态真值为拥挤，GTSEP-DL 识别为堵塞，是 5 个场景中唯一一例误判，说明在拥挤向堵塞过渡的临界区间，速度与密度特征高度相近，模型判别略偏保守。

（2）2 号场景（V = 20.97 km·h⁻¹，D = 0.529 veh·m⁻¹，N = 74）：车流密集、车速较低，同一车辆位移较小，呈典型拥挤特征；GTSEP-DL 正确识别为拥挤。

（3）3 号场景（V = 20.66 km·h⁻¹，D = 0.521 veh·m⁻¹，N = 73）：交通参数与 2 号场景相近，仍处于拥挤状态，GTSEP-DL 识别正确。

（4）4 号场景（V = 27.46 km·h⁻¹，D = 0.514 veh·m⁻¹，N = 72）：车辆密度仍较高，但平均车速明显回升，箭头所示车辆位移增大，处于缓行状态；GTSEP-DL 正确识别为缓行。

（5）5 号场景（V = 31.80 km·h⁻¹，D = 0.457 veh·m⁻¹，N = 64）：车速最高、密度最低，车辆运动顺畅，为畅通状态；GTSEP-DL 识别正确。
...
由表可见，5 个典型场景中 GTSEP-DL 正确识别 4 个，唯一误判出现在拥挤/堵塞临界的 1 号场景。整体来看，随着平均车速上升、车辆密度下降，交通状态由拥挤经缓行向畅通过渡，GTSEP-DL 的识别结果与交通参数的变化趋势一致，验证了模型在真实无人机场景下对交通状态的有效辨识能力；同时该实例也揭示了相邻拥堵等级在临界区间特征高度重叠、可分性下降的客观难点。
