...
为直观展示 GTSEP-DL 在真实无人机场景下的交通状态识别效果，从 XAM-N-6 测试集中选取 5 个典型场景。每个场景给出相隔 5 s 的两帧无人机俯拍图像（上帧为 t₁、下帧为 t₂），箭头连接的矩形框标识同一辆车（由车辆跟踪 ID 关联），用以呈现该车辆在此时段内的运动情况；图中同时标注该场景的状态真值与 GTSEP-DL 的识别结果。各场景图像如图 3-1～图 3-5 所示。

| 场景 | 图像 | 同一车辆 ID |
|---:|---|---:|
| 场景 1 | ![场景1](../outputs/figures/scenes/scene_1.png) | 702 |
| 场景 2 | ![场景2](../outputs/figures/scenes/scene_2.png) | 731 |
| 场景 3 | ![场景3](../outputs/figures/scenes/scene_3.png) | 771 |
| 场景 4 | ![场景4](../outputs/figures/scenes/scene_4.png) | 797 |
| 场景 5 | ![场景5](../outputs/figures/scenes/scene_5.png) | 860 |

各场景的交通参数与识别结果分析如下。

（1）1 号场景（V = 21.23 km·h⁻¹，D = 0.571 veh·m⁻¹，N = 80）：该时段车辆密度与车辆数在五个场景中均为最高、整体占有率最大，平均车速也处于较低水平，已接近拥挤与堵塞的判别边界。状态真值为拥挤，GTSEP-DL 识别为堵塞，是 5 个场景中唯一一例误判；由于该场景密度与占有率最高，模型在拥挤向堵塞过渡的临界区间判别略偏保守。

（2）2 号场景（V = 20.97 km·h⁻¹，D = 0.529 veh·m⁻¹，N = 74）：车流密集、车速较低，呈典型拥挤特征；GTSEP-DL 正确识别为拥挤。

（3）3 号场景（V = 20.66 km·h⁻¹，D = 0.521 veh·m⁻¹，N = 73）：平均车速在五个场景中最低，交通参数与 2 号场景相近，仍处于拥挤状态，GTSEP-DL 识别正确。

（4）4 号场景（V = 27.46 km·h⁻¹，D = 0.514 veh·m⁻¹，N = 72）：车辆密度与拥挤场景相近，但平均车速明显回升至约 27 km·h⁻¹，处于缓行状态；GTSEP-DL 正确识别为缓行。

（5）5 号场景（V = 31.80 km·h⁻¹，D = 0.457 veh·m⁻¹，N = 64）：车速在五个场景中最高、密度最低，相对最为通畅，为（相对意义上的）畅通状态；GTSEP-DL 识别正确。

各场景的交通参数与识别结果汇总如下表。

| 场景 | 时刻/s | 同一车辆 ID | 平均速度 V /(km·h⁻¹) | 密度 D /(veh·m⁻¹) | 车辆数 N | 状态真值 | GTSEP-DL 预测 | 识别结果 |
|---:|---|---:|---:|---:|---:|---|---|:---:|
| 1 | 296→301 | 702 | 21.23 | 0.571 | 80 | 拥挤 | 堵塞 | × |
| 2 | 305→310 | 731 | 20.97 | 0.529 | 74 | 拥挤 | 拥挤 | √ |
| 3 | 319→324 | 771 | 20.66 | 0.521 | 73 | 拥挤 | 拥挤 | √ |
| 4 | 329→334 | 797 | 27.46 | 0.514 | 72 | 缓行 | 缓行 | √ |
| 5 | 349→354 | 860 | 31.80 | 0.457 | 64 | 畅通 | 畅通 | √ |

由表可见，5 个典型场景中 GTSEP-DL 正确识别 4 个，唯一误判出现在密度与占有率最高、处于拥挤/堵塞临界的 1 号场景。需要说明的是，五个场景的车辆密度差异较小（0.457—0.571 veh·m⁻¹），相邻交通状态的主要区分特征是平均车速：随着平均车速由约 21 km·h⁻¹ 上升至约 32 km·h⁻¹，交通状态由拥挤经缓行向畅通过渡，GTSEP-DL 的识别结果与该趋势一致，验证了模型在真实无人机场景下对交通状态的有效辨识能力；同时该实例也揭示了相邻拥堵等级在临界区间特征高度重叠、可分性下降的客观难点。
