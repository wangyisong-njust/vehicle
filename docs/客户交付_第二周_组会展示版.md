...
本周完成"LSTM + XGBoost 加权融合"方案彻底改造为**单模型 OBB-ST-LSTM**，并在短时（UTE 3/5/8 秒）与长时（PeMS08 5/15/30 分钟）双场景下完成 7 张关键实验图、29 项指标核验。**核心创新方法在 12 个 horizon × 指标组合中 10 个最优或并列最优**。
...
![OBB-ST-LSTM 架构](../outputs/figures/delivery/arch_obb_st_lstm.png)
...
![旧 Fusion vs 新 OBB-ST-LSTM](../outputs/figures/delivery/method_compare_radar.png)
...
|---|---|---|---|
| 3 s | **0.5589** | **0.7872** | OBB-ST-LSTM 双优 ★★ |
| 5 s | 0.3678 | 0.6196 | XGBoost Macro-F1；OBB-ST-LSTM Accuracy 并列 |
| 8 s | **0.4147** | **0.6180** | OBB-ST-LSTM 双优 ★★ |

- 红色柱（OBB-ST-LSTM）在 3 个步长的 6 个柱中 **5 个最高**
...
### 关键图 ⑤ OBB-ST-LSTM 混淆矩阵

![OBB-ST-LSTM 混淆矩阵](../outputs/figures/cm_obb_st_lstm.png)
...
- **绿色柱（Ours-ST-LSTM）在 12 个柱中 12 个最低** —— 完全碾压
- 关键对比：
...
- Ours-ST-LSTM 6/6 全部最低 MAE/RMSE
...
- HF-GO 算子是 §2.4 OBB-ST-LSTM 空间张量通道 0 的直接来源
...
| `cm_obb_st_lstm.png` | OBB-ST-LSTM 混淆矩阵 |
| `obb_st_lstm_ablation.png` | 详版消融柱状图 |
...
| `docs/OBB-ST-LSTM_方案详解.md` | 工程报告口径的方案详解 |
...
| OBB-ST-LSTM 模型部署量化（ONNX / TensorRT） | 2 天 | 工程交付实时性证据 |
