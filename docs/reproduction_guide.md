...
| 实验任务 | 数据来源 | 划分方式 |
|---|---|---|
| 当前状态识别 | XAM-N-6 | 75% 训练、25% 测试，按四类状态分层随机划分 |
| 消融实验 | XAM-N-6 | 5 折分层交叉验证 |
| 参数敏感性 | XAM-N-6 | 5 折交叉验证 |
| 未来状态预测 | XAM-N-6 | 按时间顺序前 75% 训练、后 25% 测试 |
| LSTM/Fusion | XAM-N-6 | 训练段后 20% 作为验证段，用于融合权重选择 |
| 恶化预测 | XAM-N-6 | 按时间顺序前 75% 训练、后 25% 测试 |
...
- 当前状态标签是规则构造的参考标签（4 类，基于速度比/密度/占有率的分位数切分），不是人工逐窗口真值；
