...
- 近年交通流/速度预测文献常报告 15/30/60 分钟，但这些工作通常基于 PeMS、METR-LA 或固定检测器长时间序列；本项目主数据 XAM-N-6 约 5.5 分钟，不能把 15/30 分钟写成可复现实验结论。
- 如需和长时预测论文完全同口径对比，建议另建 PeMS/METR-LA 扩展实验：预测对象改为流量或速度，步长设为 15/30/60 分钟；但这类数据通常没有 pixel 表和车辆框，不能验证本项目的 HBB→OBB、HF-GO 和车辆微观扰动特征。
- 扩展实验可使用 PeMSD4/PeMSD8、METR-LA 或 PEMS-BAY，模型基线建议包含 HA、ARIMA、SVR、LSTM、GRU、DCRNN、STGCN、GraphWaveNet、TYRE 等；这属于“长时交通流/速度预测”补充，不应替代 UTE 主实验。
