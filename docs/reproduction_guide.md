...
- 为与长时预测论文同口径补充，另建 PeMS08 扩展实验：主任务为 flow/speed 连续值回归，预测步长为 5/15/30 分钟，评价指标为 MAE / RMSE。该任务对比 Persistence、RidgeLag、LSTM-deep、GRU-deep 和 Ours-ST-LSTM。PeMS08 没有 pixel 表和车辆框，不能验证本项目的 HBB→OBB、HF-GO 和车辆微观扰动特征，只用于验证预测骨架在固定检测器宏观序列上的长时适配能力。
...
| GTSEP-DL Macro-F1（3s 预测） | ~0.5417 |
| GTSEP-DL Accuracy（3s 预测） | ~0.8191 |
| GTSEP-DL 短时多 horizon 扫描 3/5/8s | 3/3 步长均为 Macro-F1 最优 |
| GTSEP-DL 5s Macro-F1 / Accuracy | ~0.5355 / ~0.8043 |
| GTSEP-DL 8s Macro-F1 / Accuracy | ~0.4066 / ~0.5955 |
| GTSEP-DL w/o MGTI Macro-F1（3s） | ~0.5195 |
| GTSEP-DL w/o disturbance gate Macro-F1（3s） | ~0.4933 |
| PeMS08 flow MAE (5/15/30 min, Ours-ST-LSTM) | 15.129 / 16.946 / 18.290 |
| PeMS08 speed MAE (5/15/30 min, Ours-ST-LSTM) | 0.782 / 1.252 / 1.592 |
| PeMS08 长时回归领先情况 | flow×3 + speed×3 共 6 个组合 MAE/RMSE 全部最优 |
| PeMS08 长时状态分类补充 Macro-F1 (5/15/30 min) | 0.8624 / 0.7712 / 0.7103 |
...
3. 依赖版本是否变化；
4. 是否只重跑了部分脚本；
5. 新版结果报告 `docs/扰动门控LSTM_实验报告.md` 是否已同步当前 JSON 结果。
...
| 新版短时结果没有更新 | 重新运行 `python scripts/03_run_experiments.py`，并确认 `outputs/reports/experiment_results.json` 已更新 |
| PeMS08 长时结果没有更新 | 重新运行 `python scripts/07_run_long_horizon_forecasting.py --dataset PEMS08 --auto-download` |
