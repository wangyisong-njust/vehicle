...
4. 提出 **GTSEP-DL** 单模型未来预测：前端 2 层卷积编码 4 通道空间张量并与标量描述符按帧拼接，单 LSTM 主干预测 3 秒后的四类状态；包含 5 组消融与 3 种子稳健性；
...
- `outputs/features/*_grid_tensors.npz`：每窗口 4×4×12 OBB 网格张量（GTSEP-DL 输入）
- `docs/experiment_report.md`：完整实验报告
- `outputs/figures/`：论文图表与 OBB 抽帧可视化（含 `cm_gtsep_dl.png`、`gtsep_dl_ablation.png`）
